          ...
          <div className="text-center max-w-3xl mx-auto">
            ...
            <form onSubmit={handleSearch} className="relative max-w-2xl mx-auto">
              <div className="relative flex items-center">
                <input type="text" placeholder="Search for restaurants or dishes..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
      ...
      <section className="py-12 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <PromotionCard icon={Percent} title="50% OFF" description="On your first order above ₹299" color="bg-gradient-to-br from-accent-500 to-accent-600" />
            <PromotionCard icon={Flame} title="Free Delivery" description="On orders above ₹199" color="bg-gradient-to-br from-primary-500 to-primary-600" />
  ...
  <Link to={`/restaurant/${restaurant.id}`} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 card-hover">
    <div className="relative h-48 overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-4">
        <p className="text-white text-sm font-medium flex items-center"><Clock className="w-4 h-4 mr-1" />30-40 min • ₹50 delivery</p>
      </div>
    </div>
    <div className="p-4">
      ...
      <div className="flex items-center justify-between mt-3">
        <span className="text-sm text-gray-500 flex items-center"><MapPin className="w-4 h-4 mr-1" />{restaurant.location}</span>
...
const PromotionCard = ({ icon: Icon, title, description, color }) => (
  <div className={`${color} rounded-2xl p-6 text-white shadow-lg transform hover:scale-105 transition-transform duration-300`}>
    <div className="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center mb-4"><Icon className="w-6 h-6" /></div>
